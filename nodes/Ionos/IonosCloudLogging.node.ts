...
export class IonosCloudLogging implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'IONOS Cloud Logging',
		name: 'ionosCloudLogging',
		icon: 'file:ionos.svg',
		group: ['transform'],
		version: 1,
		...
		properties: [
			...
			// Pipeline Name
			{
				...
				description: 'The name of the pipeline (max 20 characters)',
			},
			...
			// Logs configuration
			{
				...
				default: {},
				description: 'Log sources and destinations configuration',
				options: [
					{
						name: 'logValues',
						...
						values: [
							...
							},
							{
								...
								default: '',
								placeholder: 'app1',
								description: 'Short alphanumeric badge (3-20 characters, letters and digits only)',
							},
							{
								...
								options: [
									{
										...
										values: [
											...
											},
											{
												...
												default: 30,
												description: 'The retention period of the logs in days',
											},
										],
									},
								],
			...
			// ====================
			// Central Logging Fields
			// ====================

			// Central Logging ID
			{
				displayName: 'Central Logging ID',
				name: 'centralId',
				type: 'string',
				required: true,
				displayOptions: {
				...
				default: '',
				description: 'The ID (UUID) of the Central Logging configuration',
			},
			...
			// Central Logging Enabled
			{
				...
				default: false,
				description: 'Whether central logging is enabled',
			},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const baseUrl = `https://logging.${location}.ionos.com`;
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'pipeline') {
					if (operation === 'create') {
						...
						if (logs.logValues && Array.isArray(logs.logValues)) {
							for (const log of logs.logValues as IDataObject[]) {
								const destinations: IDataObject[] = [];
								if (log.destinations) {
									const destData = log.destinations as IDataObject;
									if (destData.destinationValues && Array.isArray(destData.destinationValues)) {
										for (const dest of destData.destinationValues as IDataObject[]) {
											destinations.push({
												type: dest.type,
												retentionInDays: dest.retentionInDays,
											});
										}
									}
								}
				...
				else if (resource === 'centralLogging') {
					if (operation === 'get') {
